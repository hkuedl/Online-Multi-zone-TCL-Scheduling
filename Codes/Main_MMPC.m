...
%% Online SO - MMPC
% Multi-resolution MPC setting:
% 0-20 min: 5-min time step, i.e., 4 fine stages
% 20-120 min: 50-min time step, i.e., each coarse stage keeps the same control for 10 fine 5-min intervals
rng(42, 'twister');
N_scen = 5;  %15
H = 24;              % total prediction horizon in original 5-min intervals, 24 = 2 h
H_fine = 4;          % first 20 min uses 5-min steps
H_coarse_step = 10;  % later horizon uses 50-min steps = 10 original 5-min intervals

mu_0 = 0;
sigma_0 = 0.01;

MMPC_Cost = zeros(N_days,3);
MMPC_Time = zeros(N_days,1);
...
for day_i = 1:10
    ...
    for t_step = 1:T
        if mod(t_step, 24) == 0
            fprintf('Solving MMPC Step %d/%d...\n', t_step, T);
        end
        % --- 1. Mapping 100 zones to equivalent clusters ---
        ...
        % --- 2. Multi-resolution MPC ---
        current_H = min(H, T - t_step + 1);  % horizon length counted by original 5-min intervals
        seg_len = [];
        seg_start = [];
        k0 = 1;
        while k0 <= current_H
            seg_start = [seg_start, k0];
            if k0 <= H_fine
                L = 1;  % near-term: 5-min resolution
            else
                L = min(H_coarse_step, current_H - k0 + 1);  % distant horizon: coarse resolution, truncated near day end
            end
            seg_len = [seg_len, L];
            k0 = k0 + L;
        end
        N_seg = length(seg_len);
        n_state = 5*num_clusters;

        % v_c_p: one control variable per segment.
        % v_c_tem: one temperature state per segment endpoint, not every original 5-min interval.
        v_c_p = sdpvar(n_state, N_seg, 'full');
        v_c_tem = cell(N_scen, 1);
        for s = 1:N_scen
            v_c_tem{s} = sdpvar(n_state, N_seg + 1, 'full');
        end
        ...
        for s = 1:N_scen
            ...
            for m = 1:N_seg
                L = seg_len(m);
                k_start = seg_start(m);
                fine_idx = k_start:(k_start + L - 1);
                abs_idx = t_step + fine_idx - 1;

                % Same control is applied throughout this segment; it must satisfy all power bounds inside the segment.
                p_lb_seg = max(P_c_min(:, abs_idx), [], 2);
                p_ub_seg = min(P_c_max(:, abs_idx), [], 2);
                Constraints = [Constraints, p_lb_seg <= v_c_p(:, m) <= p_ub_seg];

                % Coarse propagation over L original 5-min intervals:
                % x_{k+L} = Eta^L*x_k + sum_{q=1}^L Eta^(L-q)*(Beta*p + Delta_q + error_q)
                eta_L = Eta_.^L;
                beta_gain = Beta_ .* sum(Eta_.^(0:L-1), 2);
                dist_sum = zeros(n_state, 1);
                for q = 1:L
                    abs_q = abs_idx(q);
                    dist_q = reshape(Delta_new_(abs_q, day_i, :), [], 1) + scen_error(:, abs_q, s);
                    dist_sum = dist_sum + (Eta_.^(L-q)) .* dist_q;
                end
                Constraints = [Constraints, v_c_tem{s}(:, m+1) == eta_L .* v_c_tem{s}(:, m) + beta_gain .* v_c_p(:, m) + dist_sum];

                % Temperature constraints are imposed only at segment endpoints.
                k_end_abs = abs_idx(end);
                Constraints = [Constraints, T_c_ref_min(:, k_end_abs) <= v_c_tem{s}(:, m+1) <= T_c_ref_max(:, k_end_abs)];

                % Electricity cost is still accumulated over all original 5-min intervals in the segment.
                cost_ele_s = cost_ele_s + (1/12) * sum(scen_price(abs_idx, s)) * sum(v_c_p(:, m));
                % Comfort cost is approximated at segment endpoints and weighted by segment length.
                cost_comfort_s = cost_comfort_s + U_pen * L * sum(v_c_tem{s}(:, m+1).^2);
            end
            Objective = Objective + (1/N_scen) * (cost_ele_s + cost_comfort_s);
        end
        optimize(Constraints, Objective, sdpsettings('solver','gurobi','verbose',0));
        
        % Only the first 5-min decision is implemented in receding-horizon control.
    ...
    fprintf('Online MMPC Real Ele Cost: %.4f\n', real_total_cost_ele);
    fprintf('Online MMPC Real Comfort Cost: %.4f\n', real_total_cost_comfort);
    fprintf('Sum: %.4f\n', real_total_cost_ele + real_total_cost_comfort);
    MMPC_Cost(day_i,1) = real_total_cost_ele;
    MMPC_Cost(day_i,2) = real_total_cost_comfort;
    MMPC_Cost(day_i,3) = real_total_cost_ele + real_total_cost_comfort;
    MMPC_Time(day_i,1) = time_elapsed_online;
    if day_i == 4
        aa = all_tem_real_100z(:,2:end)';
        save('Tem_MMPC.mat','aa');
    end
end
